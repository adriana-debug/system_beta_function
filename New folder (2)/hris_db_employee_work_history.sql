...
CREATE TABLE `employee_work_history` (
  `work_history_id` int unsigned NOT NULL AUTO_INCREMENT,
  ...
  `reference_contact` varchar(150) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`work_history_id`),
  KEY `idx_work_history_employee` (`employee_id`),
  CONSTRAINT `employee_work_history_ibfk_1` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`) ON DELETE CASCADE
