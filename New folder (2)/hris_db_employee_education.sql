...
CREATE TABLE `employee_education` (
  ...
  `education_level` enum('Elementary','High School','Vocational','College','Masters','Doctorate') COLLATE utf8mb4_unicode_ci NOT NULL,
  `school_name` varchar(200) COLLATE utf8mb4_unicode_ci NOT NULL,
  `course_program` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `year_started` int DEFAULT NULL,
  `year_graduated` int DEFAULT NULL,
  `is_graduated` tinyint(1) DEFAULT '0',
  `honors_awards` varchar(200) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`education_id`),
  KEY `idx_education_employee` (`employee_id`),
  CONSTRAINT `employee_education_ibfk_1` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`) ON DELETE CASCADE
