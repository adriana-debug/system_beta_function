...
CREATE TABLE `document_attachments` (
  ...
  `attachment_type` enum('Supporting Document','Evidence','Signature','Photo','Certificate','Other') COLLATE utf8mb4_unicode_ci DEFAULT 'Supporting Document',
  `uploaded_by` int unsigned DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`attachment_id`),
  KEY `uploaded_by` (`uploaded_by`),
  KEY `idx_attachment_document` (`document_id`),
  CONSTRAINT `document_attachments_ibfk_1` FOREIGN KEY (`document_id`) REFERENCES `documents` (`document_id`) ON DELETE CASCADE,
  CONSTRAINT `document_attachments_ibfk_2` FOREIGN KEY (`uploaded_by`) REFERENCES `employees` (`employee_id`) ON DELETE SET NULL
