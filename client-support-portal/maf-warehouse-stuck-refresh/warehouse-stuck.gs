...
var AS_MAX_PAGES_PER_WINDOW = 8; // cap pages per slice to avoid timeouts
...
function syncDeliveryTrackingAftershipLookupWindowed(windowDaysOpt, totalDaysOpt) {
  ...
  for (let toDays = startToDays; toDays < totalDays; toDays += windowDays) {
    ...
    const orderDetails = (result && result.orderDetails) ? result.orderDetails : [];
    ...
    if (orderDetails.length) orderDetailsAll.push.apply(orderDetailsAll, orderDetails);
    windowsProcessed++;
  }
  ...
  return { slices, mergedSheetName: mergedName, orderDetails: orderDetailsAll };
}

/**                                                                                                                                             
 * Summarize completed orders (AfterShip lookup) that are not yet delivered; no emails are sent.                                                
 */                                                                                                                                             
function emailAftershipLookupOrders(runSync = false) {
  const cfg = getResolvedConfig();
  const ss = SpreadsheetApp.openById(cfg.spreadsheetId);
  const recipients = ['adrian@makeafort.fun'];
  let syncResult = null;
  if (runSync) {
  ...
  const orderDetails = (syncResult && Array.isArray(syncResult.orderDetails)) ? syncResult.orderDetails : [];
  const now = new Date();
  ...
  if (!inTransitRows.length) {
    ...
    hideNonWarehouseSheets([targetSheet]);
    return;
  }
  ...
  const toDate = (val) => val ? new Date(val) : null;
  const oldestRows = inTransitRows
    .map((row) => {
      const ts = toDate(row.inTransit) || toDate(row.infoReceived) || now;
      const deltaMs = now - ts;
      return { ...row, ageMs: deltaMs, lastMovement: ts };
    })
    .sort((a, b) => b.ageMs - a.ageMs)
    .slice(0, 10);

  const oldestTableRows = oldestRows.map((row) => `
    <tr>
      <td>${row.orderId || ''}</td>
      ...
      <td>${formatDuration(row.ageMs)}</td>
  ...
  <html>
    ...
    <body style="margin:0; padding:12px; background:#fafbfc; font-family: Arial, sans-serif; font-size: 14px; color: #0f172a;">
      ...
      <p style="margin: 0 0 12px 0; font-size: 12px; color: #334155;">Showing the 10 oldest pending orders (oldest first). Full list is in the sheet.</p>
      ...
      <table cellpadding="0" cellspacing="0" width="100%" style="border-collapse: collapse; margin: 0 0 12px 0; font-size: 13px;">
        <thead>
          <tr style="background:#eef2f7; color:#0f172a;">
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Order ID</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Channel Order ID</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Woo Status</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Tracking</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Carrier</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Country</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">State/City</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Label Created</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Age</th>
            <th align="left" style="border: 1px solid #d0d7de; padding: 6px 8px; font-weight: 600;">Last Movement (UTC)</th>
          </tr>
        </thead>
        <tbody>${oldestTableRows}</tbody>
      </table>
...
function buildAftershipIssues(orderDetails, now) {
  ...
  const issuesHtml = [
    makeTable(`Stuck labels (no movement 48h+, showing up to 20)`, stuckDetailed.slice(0, 20), ['Order ID', 'Channel Order ID', 'Woo Status', 'Original Tracking', 'Replacement / Reship', 'Carrier', 'Country', 'State/City', 'Label Created', 'Age', 'Last Movement (UTC)', 'AfterShip Status', 'Emailed / Messaged', 'Response'], (row) => `
      <tr>
...
function writeStalledSheet(ss, stalledRows, stalledThresholdHours) {
  ...
  const sheet = ss.getSheetByName(name) || ss.insertSheet(name);
  sheet.clearContents();
  const headers = [['Order ID', 'Channel Order ID', 'Woo Status', 'Tracking', 'Replacement / Reship', 'Carrier', 'Country', 'State/City', 'Label Created (CST)', `Age (>=${stalledThresholdHours}h)`, 'Last Movement (UTC)']];
  sheet.getRange(1, 1, 1, headers[0].length).setValues(headers).setFontWeight('bold');
  ...
  const rows = stalledRows.map((row) => [
    ...
    row.lastMovement ? Utilities.formatDate(row.lastMovement, "UTC", "yyyy-MM-dd HH:mm") : ''
  ]);
  sheet.getRange(2, 1, rows.length, headers[0].length).setValues(rows);
...
function getResolvedConfig() {
  ...
  return { websiteUrl, wooKey, wooSecret, aftershipKey, spreadsheetId, deliverySheetName, daysToCheck, amazonChannelIds, customerName, stalledLabelDays, csrNotesSheetId, csrNotesSheetName, aftershipThrottleMs, pageThrottleMs };
}
...
/**
 * Hide all sheets except key warehouse dashboards.
 */
function hideNonWarehouseSheets(extraKeepSheets) {
  const ss = SpreadsheetApp.openById(getResolvedConfig().spreadsheetId);
  const keep = new Set(['Website + Amazon Fulfillment Lead Time', 'Stalled Labels']);
  if (Array.isArray(extraKeepSheets)) {
    extraKeepSheets.filter(Boolean).forEach((name) => keep.add(name));
  }
