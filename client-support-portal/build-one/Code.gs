...
function doPost(e) {
  try {
    if (!e) {
      throw new Error('Missing event data. This function must be called via the deployed web app.');
    }
    ...
    if (e.postData && e.postData.contents) {
      try {
        ...
        Logger.log('Warning: Failed to parse JSON body: ' + parseError.toString());
      }
    }
    ...
    if (action === 'deleteCoachingRecord') {
      ...
      const rowIndex = params.rowIndex || data.rowIndex;
      if (!rowIndex) {
  ...
  try {
    ...
    if (!action) {
      return HtmlService.createHtmlOutputFromFile('index')
        .setTitle('Employee Performance & Disciplinary Logs')
...
function parseEmailList(value) {
  if (!value || typeof value !== 'string') return { list: [], invalid: [] };
  const list = value
    .split(/[;,]+/)
    .map(item => item.trim())
    .filter(Boolean);
  const invalid = list.filter(item => !isValidEmail(item));
  return { list, invalid };
}
...
// Create a PDF attachment from row data (HTML-based to match preview layout)
function buildPdfAttachment(rowData) {
  ...
  for (var key in rowData) {
    if (rowData.hasOwnProperty(key)) {
      rowsHtml += '<tr><th>' + escapeHtml(key) + '</th><td>' + escapeHtml(rowData[key]) + '</td></tr>';
    }
  }
  ...
  var html = ''
    ...
    + 'th{width:35%;background:#f5f5f5;text-align:left;}'
    + '</style></head><body>'
    + '<h1>Coaching Record Details</h1>'
    + '<table>' + rowsHtml + '</table>'
    + '</body></html>';

  var htmlBlob = Utilities.newBlob(html, MimeType.HTML, 'Coaching_Record.html');
...
function buildPdfAttachmentFromHtml(html, fileName) {
  var safeName = (fileName || 'Document').toString().trim();
  if (!safeName) safeName = 'Document';
  safeName = safeName.replace(/[\\\\/:*?"<>|]+/g, '-');

  var wrappedHtml = '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>' + html + '</body></html>';
  wrappedHtml = inlineLogoImage(wrappedHtml);
  var htmlBlob = Utilities.newBlob(wrappedHtml, MimeType.HTML, safeName + '.html');
  var htmlFile = DriveApp.createFile(htmlBlob);
  var pdfBlob = htmlFile.getAs(MimeType.PDF);
...
function inlineLogoImage(html) {
  try {
    var logoUrl = 'https://digitalmindsbpo.com/storage/2021/11/cropped-Digital_Minds_Logo_Original.png';
    if (html.indexOf(logoUrl) === -1) return html;
    ...
    Logger.log('inlineLogoImage failed: ' + e.toString());
    return html;
  }
}

// Simple send email without PDF
function sendEmail(recipient, subject, body, cc) {
  try {
    Logger.log('Sending simple email to: ' + recipient);
    var htmlBody = buildHtmlEmailBody(body);
    var options = {
      name: 'Digital Minds BPO Reports',
      htmlBody: htmlBody
    };
    if (cc) options.cc = cc;
...
// Send email with attachments (PDF and/or signed form)
function sendEmailWithAttachments(recipient, subject, body, attachments, cc) {
  try {
    Logger.log('Sending email with attachments to: ' + recipient);
    var htmlBody = buildHtmlEmailBody(body);
    var options = {
    ...
    if (cc) options.cc = cc;
    GmailApp.sendEmail(recipient, subject, body, options);
    Logger.log('Email with attachments sent successfully to: ' + recipient);
    return { success: true, message: 'Email sent successfully!' };
  } catch (error) {
    Logger.log('Error in sendEmailWithAttachments: ' + error.toString());
    return { success: false, message: 'Error: ' + error.toString() };
  }
}

function buildHtmlEmailBody(body) {
  if (!body) return '';
...
function performEmailSend(data) {
  const recipientRaw = data.recipient || '';
  const ccRaw = data.cc || '';
  const subject = data.subject || '';
  const body = data.body || '';
  const attachPdf = data.attachPdf || false;
  const attachSignedForm = data.attachSignedForm || false;
  const rowData = data.rowData || {};
  const pdfHtml = data.pdfHtml || '';
  ...
  const recipientParsed = parseEmailList(recipientRaw);
  if (!recipientParsed.list.length) {
    return { success: false, message: 'Recipient is required.' };
  }
  if (recipientParsed.invalid.length) {
    return { success: false, message: 'Invalid recipient email address(es): ' + recipientParsed.invalid.join(', ') };
  }
  const ccParsed = parseEmailList(ccRaw);
  if (ccRaw && ccParsed.invalid.length) {
    return { success: false, message: 'Invalid CC email address(es): ' + ccParsed.invalid.join(', ') };
  }
  const recipient = recipientParsed.list.join(',');
  const cc = ccParsed.list.join(',');

  if (attachSignedForm) {
    const signedFormUrl = rowData.SignedForm || '';
    if (!signedFormUrl) {
      return { success: false, message: 'Signed Form link is missing for this record.' };
    }
  }
  ...
  if (attachPdf) {
    if (pdfHtml) {
      attachments.push(buildPdfAttachmentFromHtml(pdfHtml, pdfFileName));
    } else {
      attachments.push(buildPdfAttachment(rowData));
    }
  }
  if (attachSignedForm) {
    const signedFormUrl = rowData.SignedForm || '';
    attachments.push(getBlobForUrl(signedFormUrl));
  }

  if (attachments.length > 0) {
    return sendEmailWithAttachments(recipient, subject, body, attachments, cc);
  }
  return sendEmail(recipient, subject, body, cc);
}
...
function sendEmailApi(data) {
  try {
    ...
    const result = performEmailSend(data || {});
...
// UI handler for HtmlService (avoids fetch/CORS)
function sendEmailUi(data) {
  try {
    Logger.log('sendEmailUi called');
    return performEmailSend(data || {});
  } catch (error) {
    Logger.log('Error in sendEmailUi: ' + error.toString());
    return { success: false, message: 'Error: ' + error.toString() };
  }
}
